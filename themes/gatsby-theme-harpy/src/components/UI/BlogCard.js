  ...
  return (
    <>
      {(type === "featured" || type === 'basic') &&
        (<div sx={{ maxWidth: type === "basic" ? [null, null, "480px"] : [null, null, null, "800px"], padding: ["1.5rem", null, "2rem"], }}>
            ...
            >
              <div sx={{ position: 'absolute', width: '100%', height: ' 100%' }}>
                ...
                >
                  ...
                  {post.medium && (
                    <Img
                      sx={{ height: '100%', objectFit: 'cover', width: '100%', borderRadius: "16px" }}
          ...
          <div>
            <div>
              <Link sx={{
                ...
                marginBottom: "24px",
                color: 'white'
              }} to={`/category/${post.categories[0].slug}`}>{post.categories[0].name}</Link>
            </div>
            <div sx={{ marginBottom: "24px" }}>
              <Link sx={{ fontSize: type === "basic" ? "1.5rem" : "2.25rem" }} to={`/${post.slug}`}><h2 sx={{ height: type === "basic" ? "2.5em" : "auto", overflow: "hidden" }} title={post.title}>{post.title.length < 50 ? post.title : post.title.slice(0, 50) + "..."} </h2></Link>
            </div>
            ...
            <div sx={{ display: "flex", gap: "16px" }}>
              <div sx={{
                width: "48px", height: "48px", borderRadius: "50%", bg: "gray"
              }}>
              </div>
      ...
      {type === "sidebar" &&
        (<div sx={{ display: "flex", gap: "1.5rem", py: "0.75rem", px: "1.5rem" }}>
          ...
          <div sx={{ flex: "1 1 auto" }}>
            ...
            <div>
              <Link to={post.slug}><h2 title={post.title} sx={{ fontSize: "1.5rem" }}>{post.title}</h2></Link>
            </div>
          </div>
        </div>)}
