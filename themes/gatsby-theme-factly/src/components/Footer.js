...
function Footer({ full, className }) {
  ...
  console.log(dega.space.logo.url);
  return (
    <React.Fragment>
      {full ? (
        <footer
          id="footer"
          className={`w-full left-0 right-0 bg-gray-300 pt-8 pb-6 ${className} bottom-0`}
        >
              ...
              <div className="w-full md:w-1/2 px-4">
                <div className="order-1 lg:order-2 mb-4">
                  <a href="/">
                    <img
                      className="h-8"
                      src={dega.space.logo.url.replace(/^"(.*)"$/, '$1')}
                      alt="factly"
                    ></img>
                  </a>
                </div>
                <h5 className="text-lg mt-0 mb-2 text-gray-700">
                  FACTLY is one of the well known Data Journalism/Public Information portals in
                  India. Each news story on FACTLY is backed by factual evidence/data from official
                  sources that is either available in the public domain or that is
