        ...
        <div className="main-content w-full -mt-8">
          <div>
            <div className="flex items-start border-b p-6 bg-gray-300">
              ...
              {bannerData.length > 0 && (
                <div className="hidden none lg:flex flex-grow flex-col justify-center px-4">
                  <h3 className="text-center font-bold py-2">{bannerTitle}</h3>
                  <div className="flex justify-center px-4">
                    {bannerData.map((item, i) => (
                      <Link to={`${baseUrl}/playlist/${item.playlistId}`} className="px-6" key={i}>
                        <img src={`/${item.icon}`} alt={item.name} className="h-20" />
                      </Link>
                    ))}
                  </div>
                </div>
                ...
                return (
                  <React.Fragment key={i}>
                    ...
                    <div className="border-b flex flex-row flex-wrap px-6 justify-center sm:justify-start items-center sm:items-start">
                      {channelSection.videos.map((video, i) => (
                        <Link
                          key={i}
